...
public class Connect5 :  IGame
{
    ...
    protected string? CheckWinner()
    {
        ...
        foreach (var cord in Flags)
        {
            Debug.WriteLine(CheckLine(cord.X, cord.Y));
            
            /*string? w = CheckLine(cord.X, cord.Y);
            if (w == null) w = CheckLine(cord.Y, cord.X);
            if (w == null)
    ...
    private string? CheckLine(int x, int y)
    {
        int count = 0;
        for (int i = x-3; i < x+4; i++)
        {
            if (IsInField(i, y) && IsInField(i - 1, y))
            {
                Debug.Write(i + " " + y);
                if (Field[i, y] == Field[i - 1, y] && Field[i, y] != EmptyCell)
                {
                    count++;
                }else
                {
